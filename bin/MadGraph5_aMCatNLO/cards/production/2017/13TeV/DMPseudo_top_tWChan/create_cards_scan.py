...
def main():
##############################################
    run_dir = "/nfs/dust/cms/user/perezdan/MC_Production/genproductions/bin/MadGraph5_aMCatNLO/cards/production/2017/13TeV/DMPseudo_top_tWChan"
    out_dir = "/nfs/dust/cms/user/perezdan/MC_Production/genproductions/bin/MadGraph5_aMCatNLO/cards/production/2017/13TeV/DMPseudo_top_tWChan/parameter_scan"
    ...
    for Mphi in list([10]+[i for i in range(50,550,50)]):
        ...
        for Mchi in [1]:
            ...
            #open template card
            fin = open(f"{run_dir}/DMPseudo_top_tWChan_proc_card.dat", "rt")
            proc_card = fin.read()
            proc_card = proc_card.replace(f'DMPseudo_top_tWChan', f'DMPseudo_top_tWChan_Mchi{Mchi}_Mphi{Mphi}')
            fin.close()
            #open the point specific card 
            fin = open(f"{out_dir}/DMPseudo_top_tWChan_Mchi{Mchi}_Mphi{Mphi}_proc_card.dat", "wt")
            fin.write(proc_card)
            fin.close()
            ...
            #open the point specific card 
            fin = open(f"{out_dir}/DMPseudo_top_tWChan_Mchi{Mchi}_Mphi{Mphi}_customizecards.dat", "wt")
            fin.write(cust_card)
            fin.close()
            
            ############# copying extramodel_card ###############
            fin = open(f"{run_dir}/DMPseudo_top_tWChan_extramodels.dat", "rt")
            extra_card = fin.read()
            fin.close()
            fin = open(f"{out_dir}/DMPseudo_top_tWChan_Mchi{Mchi}_Mphi{Mphi}_extramodels.dat", "wt")
            fin.write(extra_card)
            fin.close()
            
            ############# copying run_card ###############
            fin = open(f"{run_dir}/DMPseudo_top_tWChan_run_card.dat", "rt")
            run_card = fin.read()
            fin.close()
            fin = open(f"{out_dir}/DMPseudo_top_tWChan_Mchi{Mchi}_Mphi{Mphi}_run_card.dat", "wt")
            fin.write(run_card)
            fin.close()
